...
EQUAL = 3,QUOTE = 4,COLON = 5,BACKTAG = 6,SPACE = 7, BRACKET2_R = 8} t_type;
...
void tokenizer(char *text)
{
	...
	while(text[i] != '\0') {
		...
		switch(text[i]) {
			...
			case '}':token.type = 8;token.value = text[i];tokens[i_token] = token;i_token++;break;
